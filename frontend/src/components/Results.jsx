...
const Results = () => {
  ...
  const { correctAnswers = 0, incorrectAnswers = 0, totalQuestions = 0, timer = 0, questions = [], answeredQuestions = [] } =
    location.state || {};
  ...
  const handleCorrectErrors = () => {
    ...
    navigate("/correct-errors", { state: { incorrectQuestions } });
  };
  ...
  return (
    <div className="results">
      ...
      <button onClick={() => navigate("/exam-generator")}>Volver a Exámenes por Temas</button>
              ...
              {question.shuffledOptions.map((option, idx) => {
                ...
                return (
                  <div key={idx} className={`alternativas ${className}`}>
                    <input type="radio" name={`question_${question._id}`} checked={answered && answered.option === option} readOnly />
